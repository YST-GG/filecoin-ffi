use ffi_toolkit::{
    c_str_to_pbuf, catch_panic_response, FCPResponseStatus, raw_ptr, rust_str_to_c_str};
//独自编译模块
use filecoin_api::*;
//yst correct
use filecoin_proofs_api::{
    PieceInfo, RegisteredPoStProof, RegisteredSealProof, SectorId,
...
pub unsafe extern "C" fn fil_seal_commit_phase2(
    ...
    catch_panic_response(|| {
        ...
        let scp1o = serde_json::from_slice(from_raw_parts(
            ...
            .map_err(Into::into);
        if env::var("DISABLE_REMOTE_C2").is_err() {
            //大概8M的数据量 pub phase1_output: SealCommitPhase1Output,
            ...
            let r = api_post_polling!("seal/seal_commit_phase2", &web_data);
            //将下面的 let result = filecoin_proofs_api::seal::seal_commit_phase2移动到了远程机器
            info!("response: {:?}", r);
